...
import arayuzler.kullaniciAnaSayfa 
...
class KullaniciGirisSayfa(QtWidgets.QMainWindow):
        ...
        def kullaniciaGit(self,kullanici):
            self.close()
            self.ilkSayfa = arayuzler.kullaniciAnaSayfa.KullaniciAnaSayfa(kullanici)
            self.ilkSayfa.show()
        
        def login(self):
            email = self.mail_line.text()
            sifre = self.sifre_line.text()
            kullanici = KullaniciGirisSayfa.get_user_from_db(email, sifre)
